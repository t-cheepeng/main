...
=== Resolve feature
==== Implementation
The resolve feature is used when there is a scheduling conflict that happens within ExerHealth. The flow of the program is as such:

Step 1. User enters a `schedule` command that will cause a scheduling conflict.
...
Step 2. A `ScheduleException` is thrown from `ScheduleCommand`

Step 3. The exception will propagate to `MainWindow`.

Step 4. At that point, the exception is caught and ExerHealth will prompt the user to resolve the scheduling conflict by showing the `ResolveWindow`.
